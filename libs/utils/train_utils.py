    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        if iter_idx>0 and (iter_idx % 1000 == 0):
            ...
            save_checkpoint(
                save_states,
                # mAP>bestmAP,
                True, # 现在还没有处理val数据集,储存全部的ckpt
                file_folder='/home/ubuntu/sn15_share_dir/av_deepfake/train_results/exp13/iters_ckpt',
                file_name='epoch_{:03d}_{}.pth.tar'.format(curr_epoch + 1,iter_idx)
            )
    # finish up and print
    lr = scheduler.get_last_lr()[0]
...
def valid_one_epoch(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        # TODO 目前的策略，每个batch临时对video_list内的json进行汇总，形成临时文件
        if gt_file is None:
            gt_json_list.extend(video_list)
        # forward the model (wo. grad)
        # print(video_list)
        with torch.no_grad():
            output = model(video_list) # 走的是model.inference()
            ...
            for vid_idx in range(num_vids):
                if output[vid_idx]['segments'].shape[0] > 0:
                    ...
                    print(output[vid_idx]['scores'])
                    print(output[vid_idx]['segments'])
    ...
    elif 'json' in output_file: # 这个分支
        print('json in output_file')
        if gt_file is None:
            gt_file = gt_json_list
            print('length of gt_file:',len(gt_file))
...
def inference_one_epoch(
    ...
    """Test the model on the validation set"""
    # either evaluate the results or save the results
    assert (evaluator is not None) or (output_folder is not None)
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        if (iter_idx>0 and iter_idx%5000==0):
            with open(f'{output_folder}/data_left{iter_idx}.json', 'w', encoding='utf-8') as f:
        ...
        with torch.no_grad():
            ...
            for vid_idx in range(num_vids):
                if output[vid_idx]['segments'].shape[0] > 0:
                    ...
                    results['score'].append(output[vid_idx]['scores'])
                    # print("-----")
                    # print(video_list[vid_idx]['video_id'])
                    # print(output[vid_idx]['video_cls'])
                    # print(output[vid_idx]['scores']) # segments的confidence
                    # print(output[vid_idx]['segments'])
                    # print(output[vid_idx]['labels'])
                    # print()
                    result_item = {
                        'video_id'   :video_list[vid_idx]['video_id'],
                        'video_cls'  :output[vid_idx]['video_cls'].numpy().tolist(),
                        'scores'     :output[vid_idx]['scores'].numpy().tolist(),
                        'segments'   :output[vid_idx]['segments'].numpy().tolist()
                    }
    ...
    if len(batch_results)>0:
        with open(f'{output_folder}/data_left.json', 'w', encoding='utf-8') as f:
            json.dump(batch_results, f, ensure_ascii=False, indent=4)
        batch_results = []
