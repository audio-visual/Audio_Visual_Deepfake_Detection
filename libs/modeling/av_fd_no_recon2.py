...
from .blocks import MaskedConv1D, Scale, LayerNorm, DeepInterpolator,SegmentandCls
...
@register_meta_arch("AVLocPointTransformerRecoveryNoNormNoreconTHE")
class AVPtTransformerRecovery(nn.Module):
    ...
    def __init__(
        ...
        self.segmentandCls = SegmentandCls(input_dim, norm=False)
    ...
    def forward(self, video_list):
        ...
        # print(video_list[0].keys())
        # for k,v in video_list[0]:
        #     print(k,video_list[0][k].shape)
        ...
        norm_inputs,seg_scores, cls_scores = self.segmentandCls(batched_inputs, batched_masks)
        
        # feats, masks = self.backbone(batched_inputs,norm_inputs, reco_result,batched_masks) #看网络的表现不太容易收敛
        feats, masks = self.backbone(batched_inputs,norm_inputs, batched_inputs,batched_masks)
        ...
        if self.training:
            ...
            for idx,video in enumerate(video_list):
                if video['segments'] is not None:
                    ...
                    # gt_frame_labels
                # print(video.keys())
                if video['gt_frame_labels'] is not None:
                    gt_seg_labels.append(video['gt_frame_labels'].to(self.device))
            # print('len of gt_seg_labels:',len(gt_seg_labels))
            ...
            losses = self.losses(
                ...
                norm_inputs, None, cls_scores,seg_scores,
                gt_cls_labels,gt_seg_labels, gt_offsets,gt_video_labels,vaild_idx
            )
            # print("losses return")
            return losses
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        # print('gt_label one video:',gt_label) # 0 # tensor([0], device='cuda:3')
        gt_label_one_hot = F.one_hot(
            gt_label, self.num_classes
        ).to(reg_targets.dtype)
        # print('gt_label_one_hot one video:',gt_label_one_hot) # tensor([[1.]], device='cuda:3')
        # print("gt_frame_label shape:",gt_frame_label.shape) # torch.Size([768])
    ...
    def losses(
        self, fpn_masks,
        out_cls_logits, out_offsets,
        norm_inputs,reco_result, cls_scores, seg_scores,
        gt_cls_labels, gt_seg_labels, gt_offsets,gt_video_labels,vaild_idx
    ):
        # 只记录了fake的id！！
        ...
        # print('loss_normalizer:',self.loss_normalizer)
        ...
        # print('gt_video_labels shape:',gt_video_labels.shape)
        # print('cls_scores shape',cls_scores.shape)

        gt_seg_labels = torch.stack(gt_seg_labels)
        seg_scores = seg_scores[vaild_idx]
        # print('---')
        # print('gt_seg shape',gt_seg_labels.shape)
        # print('seg_scores shape',seg_scores.shape)
        ...
        reco_seg_loss = sigmoid_focal_loss(
            seg_scores,
            gt_seg_labels,
            reduction='sum',
            gamma = 2.0
        )
        reco_seg_loss /= self.loss_normalizer*10
        ...
        # final_loss = cls_loss + reg_loss * loss_weight + reco_seg_loss + 0.1*reco_cls_loss
        final_loss = cls_loss + reg_loss * loss_weight + 0.1*reco_cls_loss
        return {'cls_loss'   : cls_loss,
                'reg_loss'   : reg_loss,
                'reco_seg_loss'  : reco_seg_loss,
