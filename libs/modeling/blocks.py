...
class Contraction(nn.Module):
    ...
    def forward(self, x,mask):
        # print("before Contraction x.shape",x.shape) #(bs,2048,768)
        x,mask = self.down_1(x,mask)
        # print("down_1 x.shape",x.shape) #(bs,256,384)
        x,mask = self.down_2(x,mask)
        # print("down_2 x.shape",x.shape) # (bs,512,192)
        x,mask = self.down_3(x,mask)
        # print("down_3 x.shape",x.shape) #(bs,1024,96)
        x,mask = self.down_4(x,mask)
        # print("down_4 x.shape",x.shape) #(bs, 2048,48)
        x,mask = self.down_5(x,mask)
        # print("down_5 x.shape",x.shape) #(bs, 256,24)
        return x, mask
...
class Expansion(nn.Module):
    ...
    def forward(self, x,mask):
        # print("before Expansion x.shape",x.shape) # (bs, 256,24)
        x,mask = self.up_1(x,mask)
        # print("up_1 x.shape",x.shape) #(bs, 2048,48)
        x,mask = self.up_2(x,mask)
        # print("up_2 x.shape",x.shape) #(bs,1024,96)
        x,mask = self.up_3(x,mask)
        # print("up_3 x.shape",x.shape) # (bs,512,192)
        x,mask = self.up_4(x,mask)
        # print("up_4 x.shape",x.shape) #(bs,256,384)
        x,mask = self.up_5(x,mask)
        # print("up_5 x.shape",x.shape) #(bs,2048,768)
        return x, mask
...
class DeepInterpolator(nn.Module):
    def __init__(self, in_channels,hidden_channels=512,num_classes=1, norm=True):
        ...
        print(f"In DeepInterpolator: in_channels:{in_channels},hidden_channels:{hidden_channels},num_classes:{num_classes}")
        self.norm = norm
    ...
    def classifier(self, x):
        ...
        # print('x-max shape:',x_max.shape) #测试了(batch_size,256,1)
    ...
    def forward(self, inputs,mask):
        ...
        # print('DeepInterpolator inputs shape:',inputs.shape) #(bs, 2048,768)
       
        feat,mask = self.contraction(inputs,mask)
        # print('DeepInterpolator contraction feat shape:',feat.shape) #(bs, 256, 768)
        cls_socres = self.classifier(feat)
        reco,mask = self.expansion(feat,mask)
        return inputs.detach(),reco, cls_socres

class Extract(nn.Module):
    def __init__(self,in_channels,hidden_dims=1024):
        super(Extract, self).__init__()
        self.down_1 = DownBlock(in_channel=in_channels, out_channel=hidden_dims,stride=1) #(bs,1024,768)
        self.down_2 = DownBlock(in_channel=hidden_dims, out_channel=hidden_dims//2,stride=1) #(bs,512,768)
        self.down_3 = DownBlock(in_channel=hidden_dims//2, out_channel=hidden_dims//4,stride=1) #(bs,256,768)
        self.down_4 = DownBlock(in_channel=hidden_dims//4, out_channel=hidden_dims//8,stride=1) #(bs,128,768)
        self.down_5 = DownBlock(in_channel=hidden_dims//8, out_channel=hidden_dims//16,stride=1)  #(bs,64,768)
...
class SegmentandCls(nn.Module):
    def __init__(self, in_channels,hidden_channels=1024,num_classes=1, norm=True):
        super(SegmentandCls, self).__init__()
        print(f"In SegmentandCls: in_channels:{in_channels},hidden_channels:{hidden_channels},num_classes:{num_classes}")
        self.norm = norm
        self.contraction = Extract(in_channels,hidden_channels)
        
        self.conv0 = nn.Sequential(
            nn.Conv1d(hidden_channels//16, hidden_channels//16, kernel_size=1, bias=False),
            nn.InstanceNorm1d(hidden_channels//16),
            nn.LeakyReLU(0.2))
        
        self.cls_linear1 = nn.Linear(num_classes*2, num_classes)

        self.seg_linear = nn.Linear(hidden_channels//16, num_classes)
        self.bn1 = LayerNorm(hidden_channels)
        self.dp1 = nn.Dropout(p=0.5) 

   
    def segment(self, x,clsloss=True):
        # print('before seg x.shape:',x.shape)
        x = self.conv0(x) #(batch_size,64,time)
        # print('conv0 seg x.shape:',x.shape)
        x = x.permute(0, 2, 1) #(batch_size,time,64)
        x = self.seg_linear(x) #(batch_size,time,2)
        # print('seg_linear seg x.shape:',x.shape) # (bs, 768,1)
        map = x.permute(0, 2, 1)#(batch_size,2,time)
        # if not clsloss:
        #     return map,None
        x_max = F.adaptive_max_pool1d(map, 1)
        x_avg = F.adaptive_avg_pool1d(map, 1)
    ...
    def forward(self, inputs,mask):
        ...
        # print('contraction feat shape:',feat.shape) #(bs, 256, 768)
        seg_scores, cls_socres = self.segment(feat)
        # reco,mask = self.expansion(feat,mask)
        return inputs.detach(),seg_scores, cls_socres    